...
int main(int argc, char* argv[]) {
  ...
  char* read_mode = "rb";
  enum modes { BASEVAL, CHECKSUM, CHECKSUM2, MANUAL } mode;
  ...
  if (argv[1][1] == 'b' && argc == 3) {
    ...
    if (argc == 4) {
      ...
      printf("User input: startval: %04x\n", startval);
    } else if (argv[1][1] == 'm' && argc == 5) {
      startval = (uint16_t)strtol(argv[3], NULL, 0);
      printf("User input: startval: %04x\n", startval);
      skip_at = (uint32_t)strtol(argv[4], NULL, 0);
      printf("User input: checksum offset: %04x\n", skip_at);
      mode = MANUAL;
    }
  } else {
...
*/
  if (mode == MANUAL) {
    printf("Manual mode, skipping any possible check..\n");
  } else {
    ...
    if (magic == BCL_MAGIC) {
      ...
      } else if (magic != CHECKSUM_MAGIC) {
        fprintf(stdout, "[INSECURE] Checksum magic is invalid (%04x).\n",
                magic & 0xFFFF);
      } else {
        printf("Detected raw Ntk firmware image (NonComp/PartComp)\n");
        format = RAW;
...
*/
  ...
  } else if (mode == CHECKSUM || mode == MANUAL) {
    printf("checksum: %04x\n", checksum);
...
void print_usage(int argc, char* argv[]) {
  ...
  fprintf(stderr, "    -c[w] <filename> <baseval> :: checksum calculation with custom start value (hex)\n");
  fprintf(stderr, "    -m[w] <filename> <baseval> <checksum offset> :: manual checksum calculation\n");
  fprintf(stderr, "\n");
...
uint16_t calc_checksum(uint16_t data[], uint32_t length, uint32_t skip_at,
                       uint16_t startval) {
  uint32_t pos = 0;
  uint16_t checksum = 0;
  while (pos < length) {
